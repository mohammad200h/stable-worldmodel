...
from env import MountainCarWorldModelEnv

@hydra.main(version_base=None, config_path='./', config_name='env_demo_pixels')
def run_demo_pixels(cfg):
    env = MountainCarWorldModelEnv(cfg.world_model_path, cfg.checkpoint)
    obs, info = env.reset()
    action = env.action_space.sample()
    next_obs, reward, done, truncated, info = env.step(action)

    print(f"next obs embedding shape: {next_obs.shape}")
...
def run_demo_state(cfg):
    env = MountainCarWorldModelEnv(cfg.world_model_path, cfg.checkpoint, embedding_is_made_of_pixels=False)
    obs, info = env.reset()
    action = env.action_space.sample()
    next_obs, reward, done, truncated, info = env.step(action)
    print(f"next obs embedding shape: {next_obs.shape}")
    print(f"next obs embedding: {next_obs}")
    print(f"reward: {reward}")
    print(f"done: {done}")
