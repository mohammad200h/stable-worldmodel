"""Evaluate a World Model on Fetch expert trajectories stored in Lance format."""
...
# LanceWriter stores episode boundaries in these columns; they are not
# exposed via LanceDataset.column_names (only data columns are listed there).
_EPISODE_COL = 'episode_idx'
_STEP_COL = 'step_idx'
...
def get_episodes_length(dataset, episodes):
    episode_idx = dataset.get_col_data(_EPISODE_COL)
    step_idx = dataset.get_col_data(_STEP_COL)
    lengths = []
...
@hydra.main(version_base=None, config_path='./config', config_name='fetch_rl')
def run(cfg: DictConfig):
    """Run WM planning evaluation on Fetch Reach expert Lance trajectories."""
    assert (
    ...
    ep_indices, _ = np.unique(
        stats_dataset.get_col_data(_EPISODE_COL), return_index=True
    )
    ...
    row_episode_idx = dataset.get_col_data(_EPISODE_COL)
    max_start_per_row = np.array(
        [max_start_idx_dict[ep_id] for ep_id in row_episode_idx]
    )

    valid_mask = dataset.get_col_data(_STEP_COL) <= max_start_per_row
    ...
    eval_episodes = row_episode_idx[random_episode_indices]
    eval_start_idx = dataset.get_col_data(_STEP_COL)[random_episode_indices]
